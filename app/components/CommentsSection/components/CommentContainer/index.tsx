...
import VerifiedIcon from '@/public/assets/Twitter_Verified_Badge.svg'; // Path to your verification icon
import {
...
export default function Comment({
    ...
    const [userVerified, setUserVerified] = useState<boolean>(false); // New state for user verification
    ...
    async function getCommentCurrLikesAndDislikes() {
        ...
        const userInfo = await getDoc(doc(db, 'users', comment.userId.id));
        setUserVerified(userInfo.get('verified') || false);
    }
            ...
            <div className={styles.comment_data}>
                <div className={styles.heading_container}>
                    <h5>
                        ...
                        {userVerified && <Image src={VerifiedIcon} alt="Verified" width={16} height={16} />} {/* Display verification icon */}
                    </h5>
...
function SvgIcons({ type, isBtnActive, commentData }: { type: "like" | "dislike" | "reply", isBtnActive: boolean, commentData: UserComment }) {
    switch (type) {
        ...
        case 'reply':
            return isBtnActive
                ? <><SvgReplyFill width={16} height={16} alt="Reply" /> {(commentData.replies?.length != 0) && commentData.replies?.length} &#x2022; Reply</>
                : <><SvgReply width={16} height={16} alt="Reply" /> {(commentData.replies?.length != 0) && commentData.replies?.length} &#x2022; Reply</>;
